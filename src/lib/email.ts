    ...
    const info = await transporter.sendMail({
      ...
      html: `
        <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; color: #333;">
          <h2 style="color: #f26c22;">Welcome to Inspire Summit 2026!</h2>
          <p>Hi ${name},</p>
          <p>Your registration is confirmed. We are thrilled to have you join us.</p>
          
          <div style="background: #f4f4f4; padding: 15px; border-radius: 8px; margin: 20px 0; border-left: 4px solid #00aeef;">
          ...
          <a href="${qrLink}" style="display: inline-block; padding: 10px 20px; background: #004b87; color: white; text-decoration: none; border-radius: 5px;">View My Ticket</a>
...
export const sendGenericEmail = async (
  ...
  if (!SMTP_HOST || !SMTP_PORT || !SMTP_USER || !SMTP_PASS) {
    console.warn("SMTP credentials missing. Mocking email send:", subject, "to", to);
    return false;
  }

  try {
    const info = await transporter.sendMail({
      from: `"INSPIRE 2026" <${SMTP_USER}>`,
      to,
      subject,
      html: htmlContent,
    });
